...
def message(request):
    ...
    if media_id and Media.objects.filter(media_id=media_id).exists():
        ...
        if Chat.objects.filter(articulo=articulo, articulo__propietario=request.user).exists(): # Caso de chat de propietario con interesado
            ...
            ava_ac = UsuarioInfo.objects.get(usuario=chat_ac.interesado)
            ...
            if Chat.objects.filter(articulo=articulo, interesado=request.user).exists():
                if Chat.objects.filter(articulo=articulo, chat_id=chat_id).exists():
                    ...
                    chat_ac = Chat.objects.get(articulo=articulo, interesado=request.user)
            else:
                chat_ac = Chat.objects.create(articulo=articulo, interesado=request.user)
            ava_ac = UsuarioInfo.objects.get(usuario=chat_ac.articulo.propietario)

    if request.method == 'POST':
        texto = request.POST.get('texto')
        if chat_ac and articulo and 0 < len(texto) <= 80 and not articulo.asignado:
    ...
    return render(request, 'gestionMensajeria/messages.html', {'chats_pr': chats_pr, 'chats_in': chats_in, 'chat_ac': chat_ac, 'ava_ac': ava_ac})